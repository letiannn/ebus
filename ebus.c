 ...
 * @return {*}
 */
static uint16_t Ebus_GetSn(void)
{
 ...
 * @return {*}
 */
static sEbusNode_t* EBus_FindNodeByName(const char *name)
{
    rt_mutex_take(g_ebus_.bus_mutex, RT_WAITING_FOREVER);
 ...
 * @return {*}
 */
static int Ebus_FindIdleIdx(void)
{
    rt_mutex_take(g_ebus_.bus_mutex, RT_WAITING_FOREVER);
    int idx = -1;
    for (int i = 0; i < EBUS_MAX_NODE_NUM; i++)
 ...
 * @return {*}
 */
static sEbusNode_t *Ebus_FindNodeByIdx(uint8_t node_idx)
{
 ...
 */
static int Ebus_FindWaitRespItem(sEbusNode_t *node, uint16_t seq_num)
{
 ...
 */
static int Ebus_AllocWaitRespItem(sEbusNode_t *node)
{
    if (node == RT_NULL || !node->init) {
 ...
 * @return {*}
 */
static void Ebus_FreeWaitRespItem(sEbusNode_t *node, int idx)
{
 ...
 * @return {*}
 */
static void Ebus_ProcessResponse(sEbusNode_t *node, sEbusMsgItem_t *msg)
{
 ...
 * @return {*}
 */
static void Ebus_BusInit(uint8_t idx, sEbusNode_t *node)
{
 ...
 * @return {*}
 */
static void Ebus_BusDeinit(uint8_t idx)
{
    if (idx < 0 || idx >= EBUS_MAX_NODE_NUM)
    {
 ...
 * @return {*}
 */
static eEbusRst_t Ebus_MsgSend(sEbusNode_t *node, sEbusMsgItem_t *msg_item)
{
    ...
    case eEbusMsgType_Indication:
        {
            sEbusNode_t* target_node = Ebus_FindNodeByIdx(msg_item->dst_node_idx);
...
void Ebus_Create(void)
{
    ...
    g_ebus_.init = 1;
    g_ebus_.node_len = 0;
    g_ebus_.bus_mutex = rt_mutex_create("ebus_mutex", RT_IPC_FLAG_FIFO);
 ...
 * @return {*}
 */
sEbusNode_t *Ebus_NodeCreate(char *name, EbusCbPtr EvtCb)
{
    ...
    rt_strncpy(node->name, name, RT_NAME_MAX - 1);
    node->name[RT_NAME_MAX - 1] = '\0';  // 确保字符串终止
    node->Evtcb = EvtCb;
    ...
    // 创建响应互斥量
    node->resp_mutex = rt_mutex_create("node_resp_mutex", RT_IPC_FLAG_FIFO);
    ...
    // 创建消息队列
    char mq_name[RT_NAME_MAX] = {0};
    rt_snprintf(mq_name, RT_NAME_MAX, "%s_mq", name);
    ...
    // 注册到总线
    Ebus_BusInit((uint8_t)idle_idx, node);

    // 检查是否初始化成功
    if(!node->init)
    {
        LOG_E("[Ebus] Failed to initialize node: %s", name);
        Ebus_BusDeinit(idle_idx);
        rt_mq_delete(node->msg_queue);
        rt_mutex_delete(node->resp_mutex);
        rt_free(node);
        return RT_NULL;
 ...
 * @return {*}
 */
void Ebus_NodeDestory(sEbusNode_t *node)
{
    if (node == RT_NULL) {
 ...
 * @return {*}
 */
eEbusRst_t Ebus_MsgWaitRecv(sEbusNode_t *node, sEbusMsgItem_t *msg, uint32_t timeout)
{
    ...
    if (len > 0) {
        ...
        if (msg->type == eEbusMsgType_Indication && node->Evtcb != RT_NULL) 
        {
            ...
            sEbusNode_t *ack_node = Ebus_FindNodeByIdx(msg->src_node_idx);
            if (ack_node != RT_NULL) {
 ...
 * @return {*}
 */
eEbusRst_t Ebus_MsgRecv(sEbusNode_t *node, sEbusMsgItem_t *msg)
{
    LOG_D("[Ebus] Try receive message: node=%s", node->name);
    return Ebus_MsgWaitRecv(node, msg, 0); 
}
 ...
 * @return {*}
 */
eEbusRst_t Ebus_Broadcast(sEbusNode_t *node, sEbusMsgItem_t *msg)
{
    if (node == RT_NULL || !node->init || msg == RT_NULL) {
    ...
    msg->seq_num = Ebus_GetSn();
    msg->timestamp = rt_tick_get();
    
    return Ebus_MsgSend(node, msg);
}
 ...
 * @return {*}
 */
eEbusRst_t Ebus_Notification(sEbusNode_t *node, char *dst_node_name, sEbusMsgItem_t *msg)
{
    ...
    sEbusNode_t* dst_node = EBus_FindNodeByName(dst_node_name);
    if (dst_node == RT_NULL) {
 ...
 * @return {*} 执行结果
 */
eEbusRst_t Ebus_IndicationAsync(sEbusNode_t *node, char *dst_node_name, sEbusMsgItem_t *msg)
{
    if (node == RT_NULL || msg == RT_NULL || dst_node_name == RT_NULL) 
    {
    ...
    // 分配等待响应项
    int wait_idx = Ebus_AllocWaitRespItem(node);
    if (wait_idx < 0) {
 ...
 * @return {*} 执行结果
 */
eEbusRst_t Ebus_Response(sEbusNode_t *node, sEbusNode_t *ack_node, sEbusMsgItem_t *msg)
{
    ...
    // 更新等待响应项状态
    int idx = Ebus_FindWaitRespItem(ack_node, msg->seq_num);
    if (idx >= 0) 
    {
    ...
    for (int node_idx = 0; node_idx < EBUS_MAX_NODE_NUM; node_idx++) 
    {
        ...
        for (int slot_idx = 0; slot_idx < EBUS_NODE_MAX_RESP_WAIT_NUM; slot_idx++) 
        {
            ...
            if (item->state != eEbusMsgState_Idle) 
            {
                ...
                switch (item->state) 
                {
                    case eEbusMsgState_Sented: state_str = "SENTED"; break;
                    case eEbusMsgState_Recved: state_str = "RECVED"; break;
